...
@section('content')
    ...
    <div class="portlet box blue">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-plus"></i>Edit Brand
            </div>
        </div>
                ...
                <div class="form-body">
                            ...
                            <select class="form-control" name="category_id">
                                @if(isset($categories))
                                    @foreach($categories as $row)
                                        <option value="{{$row->id}}" <?=$row->id==$brand['category_id']?'selected':''?>>
                                            {{$row->name}}
                                        </option>
                                    @endforeach
                                @endif
                            ...
                            <select class="form-control" name="sub_category_id">
                                @if(isset($sub_categories))
                                    @foreach($sub_categories as $row)
                                        <option value="{{$row->id}}" <?=$row->id==$brand['sub_category_id']?'selected':''?>>
                                            {{$row->name}}
                                        </option>
                                    @endforeach
                                @endif
                    ...
                    <div class="form-group">
                        ...
                        * </span>
                        ...
                        <div class="col-md-4">
                            <img src="{{ $brand['image'] }}" width="200px" height="200px">
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="">
                            <div class="input-icon right">
                                <i class="fa"></i>
                                <input type="checkbox"  <?=$brand['is_header']?'checked':""?>  class="form-control" name="is_head" value="1" />Are you want to show in Header<span class="required">
                                * </span>
                            </div>
                        </div>
                        </label>
    ...
    var handleValidation2 = function() {
            ...
            form2.validate({
                ...
                submitHandler: function (form) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '{{ url("admin/brands/update") }}/{{ $brand["id"] }}',
                        data: formData,
                        cache       : false,
                        contentType : false,
                        processData : false,
